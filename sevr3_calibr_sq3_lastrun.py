...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/arinasevruk/Desktop/Exp/MyExp/sevr3_calibr_sq3_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    loop_L = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('task_m_s4.xlsx'), 
        seed=None, 
    )
    ...
    # create an object to store info about Routine send_experiment_to_osf
    send_experiment_to_osf = data.Routine(
        name='send_experiment_to_osf',
        components=[text_7],
    )
    send_experiment_to_osf.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # store start times for send_experiment_to_osf
    send_experiment_to_osf.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    send_experiment_to_osf.tStart = globalClock.getTime(format='float')
    send_experiment_to_osf.status = STARTED
    thisExp.addData('send_experiment_to_osf.started', send_experiment_to_osf.tStart)
    send_experiment_to_osf.maxDuration = None
    # keep track of which components have finished
    send_experiment_to_osfComponents = send_experiment_to_osf.components
    for thisComponent in send_experiment_to_osf.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine and routineTimer.getTime() < 1.0:
        ...
        if thisExp.status == PAUSED:
            pauseExperiment(
                ...
                win=win, 
                timers=[routineTimer, globalClock], 
                currentRoutine=send_experiment_to_osf,
            )
            # skip the frame we paused on
            continue
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            send_experiment_to_osf.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in send_experiment_to_osf.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in send_experiment_to_osf.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for send_experiment_to_osf
    send_experiment_to_osf.tStop = globalClock.getTime(format='float')
    send_experiment_to_osf.tStopRefresh = tThisFlipGlobal
    thisExp.addData('send_experiment_to_osf.stopped', send_experiment_to_osf.tStop)
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if send_experiment_to_osf.maxDurationReached:
        routineTimer.addTime(-send_experiment_to_osf.maxDuration)
